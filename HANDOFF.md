...
## 2026-09-16 (late night) -- public repo, releases, measured phone speed
- **Public repo** github.com/lukasbaxter/conduit; releases v0.1.0-v0.1.2 built by `.github/workflows/release.yml` on a `v*` tag: macOS dmg (ad-hoc signed via `build/adhoc-sign.js`; unsigned was refused as "damaged"; `xattr -cr` clears it), Windows nsis+portable exe (`--publish never` or electron-builder tries to upload itself), Android APK (Expo prebuild + gradle, keystore in repo secrets `ANDROID_KEYSTORE_B64/_PASS`, copy in `~/.conduit-android/` and `.85:~/.conduit-android/`; the signing block is appended to app/build.gradle with values from env). Test account password now lives in `~/.conduit-test.env` (`CONDUIT_TEST_PASS`), rotated.
- **Native shell** (`mobile/`): hardware volume buttons drive the remote device while the sound is elsewhere (`react-native-volume-manager`; page<->shell bridge via ReactNativeWebView postMessage / `conduit:volumestep` event).
- **Measured on a 5 Mbps / 80 ms link** (`test/.probe/slow-net.mjs`, TEST ACCOUNT ONLY: running it as lukasbaxter made the headless browser his active player and played over him): the real costs were the keep-alive WAV preload (9.6 MB per launch), Jellyfin JPEG quality 90 covers (7 MB per album page), Jellyfin's own queries (album list 12.5 s, IsPlayed history 4 s each, favourites 20 s for the 1,404-like account, /Items?Ids= 1.5 s per 150), and Meili's 200-hit search cap silently dropping 804 liked songs. Fixes: `preload=none`, `quality=72/82` on covers, relay `/liked` (likes table + index, chunks of 200), relay `/library` (albums+artists from index DOCUMENTS, cached 10 min) and `/home` (recently played / most played from the relay's own `plays` table recorded off nowplaying + an hourly Jellyfin seed), album/artist lists reused 6 h client-side, play-history queries memoised, nginx gzip on `/relay`, HLS prewarm (next track 20 s early, rows on touchstart). Home on the slow link: 11 s -> 1.4 s.
- **iOS streaming**: transcodes are HLS on WebKit (`streamMode()` 'hls' / 'chunked' / 'file'); the chunked MP3 would not play on the iPhone over cellular. nginx shapes `/universal` (Android) to 1.5 MB then 80 KB/s.
- **Jellyfin**: Playback Reporting plugin disabled, DB VACUUM/ANALYZE'd at a restart 22:11 (album query 12.5 -> 5.4 s), lukasbaxter password set; the DbUpdateConcurrency errors were that user's stuck row and stopped after the restart. `algo-noise-browser` (his other project) burns ~5 cores; Jellyfin got cpu-shares 2048 vs 256.
- **Desktop discovery with a Tailscale exit node**: macOS routes 224.0.0.0/4 into utun9, so mDNS finds nothing (even `dns-sd`); discovery.js now pins the socket to the LAN IPv4. The installed Conduit.app also needs macOS Local Network permission. The desktop device trace file `$TMPDIR/conduit-trace.log` is shared by the dev app and the installed app.

## 2026-09-16 (night) -- car-ride optimisation, lyrics in RAM, Henry
- **Evening logs** (nginx access log, 23:00-04:00 UTC): Henry on Android (Chrome 153, Telus cellular) 16:07-17:xx PDT as `henrybaxter` (one 401 = a mistyped password first), Lukas on the iPhone 19:00-20:xx. Android did 909 x 304 image revalidations + 445 x 200 in an hour (Jellyfin answers covers with Last-Modified = now and no max-age) and pulled FLAC in 6 MB chunks.
- **Artwork cache**: nginx `00-jfimg-cache.conf` (http-level zone) + a `location ~ ^/jf/(Items|Users|Artists)/[^/]+/Images/` in `music-locations.inc`: server-side 7 d keyed by item+size+tag/bust (not api_key), client `max-age=86400, stale-while-revalidate`, `X-Cache` header. `hygiene.sh` wipes it after the nightly cover pass. Now-playing cover paints from the cached 320px copy and swaps in 640 (phone) / 1000 (desktop) (`useProgressiveSrc` in FullScreen.jsx).
- **Phone streaming**: `prefs.phoneQuality` (default `high` = 320 kbps MP3 transcode) beside the account `quality`; App.jsx `deviceQuality()` sets `jf.quality` per device; Settings has "on phones" / "elsewhere" rows. Transcodes have no byte ranges, so usePlayer keeps `localBaseRef` (stream start offset), `seek` restarts the transcode at `startTimeTicks`, position = base + element time, Jellyfin's track length stays authoritative. Probe `test/.probe/transcode-seek.mjs`.
- **Transcode rate shaping** (nginx `location ~ ^/jf/Audio/[^/]+/universal` in music-locations.inc): Jellyfin encodes audio flat out (no throttle for audio), so a whole song arrived in one burst (Henry's evening: 6-9 MB per track, wasted on skips). Now `limit_rate_after 1500k; limit_rate 80k;` = first 1.5 MB (~37 s of 320k) at full speed, then 80 KB/s (~2x real time). GOTCHA: limit_rate is ignored with `proxy_buffering off`; that location buffers in memory (`proxy_buffers 16 64k; proxy_max_temp_file_size 0`) which also back-pressures ffmpeg. Measured: 2.5 MB in 12 s. `/stream` originals (desktop, speakers) untouched.
- **Android back in the open player**: now playing is its own `history.pushState({np:true})` step; popstate closes it and stays on the album/playlist (`fullScreenRef` in App.jsx). Probe `test/.probe/back-np.mjs`.
- **Lyrics in RAM**: `conduit-redis` container (redis:7-alpine, no persistence, 1 GB cap) beside the relay; `relay/lyrics.js` loads every `.lrc` sidecar (id = Jellyfin's MD5-of-path, `jellyfinId()`) + Jellyfin's own `metadata/library/<xx>/<id>/*.lrc` (3,801 internal-only) = 21,548 tracks in ~4 s at start (when older than a day) and on `POST /lyrics/reload`. `GET /relay/lyrics?id=` (user token) answers in ~2 ms in Jellyfin's `{Lyrics:[{Start,Text}]}` shape; the client (`jf.lyrics`) asks the relay first, Jellyfin as fallback. Mounts: `/mnt/wd_nvme1/music:/music:ro`, `.../jellyfin/config/metadata/library:/jfmeta:ro`.
- **Lyrics data fixes**: `lyrics_check.py` full pass: 16,239 checked, 2,393 sidecars replaced by LrcLib's duration-matched record (backups in `~/conduit-hygiene/lyrics_backup/`), Jellyfin refreshed. `lyrics_orphans.py`: the 2026-09-11 retag renamed audio files ("02. Milky Chance - Ego.flac" -> "02. Ego.flac") and left 2,956 `.lrc` orphans; 1,474 re-attached by track number (1,410 unmatched are mostly junk at the library root). Both + `refresh_lyrics.py` + the Redis reload run in nightly `hygiene.sh`.
- **henryb / henrybaxter**: new Jellyfin user `henryb` (Music+Playlists+Collections/Movies/Shows). `henrybaxter` (existing admin) was given Music+Playlists and got his portal Spotify data mirrored in: 16 playlists, 1,053/1,729 tracks matched, 513 favourites; nightly cron 05:35 (`spotify_to_jellyfin.py henrybaxter henrybaxter`). Music Requests portal: henrybaxter account exists (id 3, Spotify linked 2026-06-30, never logged in); the portal still creates Navidrome users, not Jellyfin ones.
- **Direction (Lukas)**: eventually ONE docker container that takes a mounted music library and just works, other services feeding it music; drop Jellyfin if real streaming needs it. Not started; everything above still runs on Jellyfin.

## 2026-09-16 (afternoon) -- iPad, lock screen, visualizer
- **Visualizer shadow stream is decoded, not a captured <audio>** (`src/api/shadowStream.js`): the mirroring device's hidden `<audio>` + createMediaElementSource was AUDIBLE on the iPad (song on both devices). Now the mp3 transcode is fetched as bytes, cut at frame boundaries, decodeAudioData'd and scheduled on the context clock into a node with no path to the speakers; frame-counted time, restart on > 0.6 s drift. Calibrate reads `sh.position`/`sh.paused`. Probe: `test/.probe/viz-mirror.mjs` (mirror's bars move, zero <audio> elements).
- **iPad = desktop layout on a touch screen**: row tap plays on `(pointer: coarse)` (`useTouch()` in TrackRow); every `:hover` rule is wrapped in `@media (hover: hover)` by `postcss.config.js` (WebKit's hover emulation ate the first tap wherever hover revealed a play button -> double taps).
- **Media Session for the whole session** (usePlayer, `msLocal`/`msRemote`): lock screen / Now Playing show and control the track playing on another client or a speaker driven from here. A silent keep-alive element (`src/player/keepalive.js`) stands in for the media the OS wants to see; it is `keepalive.wav` (20 min of 8 kHz 8-bit PCM, emitted at build by `tools/vite-keepalive.js`, range-served by nginx `location = /keepalive.wav`, immutable) and is kept within 4 s of the session position. **iPhone gotchas found with the relay diag (`keepAlive` field in the phone diag):** (1) a 1-s loop snaps the lock-screen time to 0 every second (iOS reads the element's time as well as setPositionState); (2) a seek applied BEFORE the first play (at loadedmetadata) freezes the element's clock for good on the iPhone (blob or URL, mp3 or WAV alike; the iPad was fine) -> start at 0, seek only a clock seen moving, freeze detector leaves a stuck clock alone; (3) iOS needs one tap on the page before the keep-alive may start; (4) remote pauses < 2 s do not stop it (every pause/play is a 0 on the lock screen). Lock-screen LIKE is impossible for a web app (no Media Session action).
- **Phone seek bar** follows the touch's own x (iOS native range drag only sometimes followed the thumb).
- **Desktop app after Mac sleep**: the long-running Electron process came out of sleep with every LAN device call failing `EHOSTUNREACH` while a fresh process reached the Node fine -> restart `npm run dev`. If it recurs, check Privacy & Security -> Local Network -> Electron.
- Relay logs `likes reconcile The operation was aborted due to timeout` every few minutes (Jellyfin slow) -- not looked at.

## 2026-09-16 -- iOS 18 installed web app: bottom chrome flush with the screen (SOLVED, confirmed on the phone)
- The bug: standalone + `black-translucent` lays the page out one status bar short (viewport 793 on an 852 screen, `safe-area-inset-top` 59) while the web view is full screen. WebKit treats the strip below like the area under a Safari toolbar: page content IS painted there, viewport-fixed layers are CLIPPED at 793 (pushing the fixed tab bar down just cut its icons in half). The opaque status bar (`black`) only hid it and iOS reads that meta at install time anyway.
- The fix (App.jsx `--ios-shim` effect + the `html.ios-shim` block at the end of `mobile-nowplaying.css`): measure `screen.height - innerHeight` (only when standalone and sat > 0), set `html { height: <screen.height>px }` and `body { transform: translate(0) }` so body becomes the containing block of every `position: fixed` layer (tab bar, mini player, sheets, now playing, drawer, portals into body). They anchor to 852 and are painted as page content. Page pinned at scroll 0, `overscroll-behavior-y: contain` on everything.
- Gotchas: (1) must LATCH: applying the fix grows the viewport to 852 -> resize -> gap 0 -> fix removed -> viewport 793 -> ... the phone flickered at frame rate; (2) the page height must be the absolute screen pixel count, not `100% + gap`, for the same reason; (3) iOS fires 1x1 / 4x4 web-view resizes around app switching, ignore widths < 200. Probe: `test/.probe/shim.mjs` (393x793 viewport, shim forced).
- Lock-screen like button: impossible from a web app (Media Session has no like action; Spotify uses native MPRemoteCommandCenter). Offered an Action Button / Shortcut -> relay "like now playing" route, not built.
